...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    -m|--makearg)
      ...
      ;;
    -?|-h|--help)
      echo 'bash gitrevc.sh [non-switched arguments in order (computer port user extension)]'
...
then
 ...
 ssh.exe -p $port "$user@$computer" "cd $remotePath/;git diff > $patchName"
 # Get the patch from remote machine
 scp.exe -P $port -o 'StrictHostKeyChecking no' "$user@$computer:$remotePath/$patchName" "$patchName"
 # Remove patch from remote machine
 ssh.exe -p $port "$user@$computer" "cd $remotePath/;rm -f $patchName"
 # Purge local diff and apply remote diff.
 git checkout .
 git apply $patchName
else
 # Create patch remotely
 ssh.exe -p $port "$user@$computer" "cd $remotePath/;git diff $fileName > $patchName"
 # Get the patch from remote machine
 scp.exe -P $port -o 'StrictHostKeyChecking no' "$user@$computer:$remotePath/$patchName" "$patchName"
 # Remove patch from remote machine
 ssh.exe -p $port "$user@$computer" "cd $remotePath/;rm -f $patchName"
