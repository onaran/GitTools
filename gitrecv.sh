...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    -e|--extension)
      ...
      echo -e '\e[31m-e|--extension \e[32mextension i.e. "\.\(c\|sh\)" file\e[0m'
      exit 0 
      ;;
  *)    # unknown option
...
echo -e '\e[33m Modified files'
git status | sed -n "s/^.*modified:\s*\(.*\).*/\1 /p"
echo -e '\e[0m'

if [[ $fileName == "" ]]
then
 # Create patch remotely
 ssh.exe -p $port "$user@$computer" "cd ~/Projects/Local/;git diff > $patchName"
 # Get the patch from remote machine
 scp.exe -P $port -o 'StrictHostKeyChecking no' "$user@$computer:~/Projects/Local/$patchName" "$patchName"
 # Remove patch from remote machine
 ssh.exe -p $port "$user@$computer" "cd ~/Projects/Local/;rm -f $patchName"
 # Purge local diff and apply remote diff.
 git checkout .
 git apply $patchName
else
 # Create patch remotely
 ssh.exe -p $port "$user@$computer" "cd ~/Projects/Local/;git diff $fileName > $patchName"
 # Get the patch from remote machine
 scp.exe -P $port -o 'StrictHostKeyChecking no' "$user@$computer:~/Projects/Local/$patchName" "$patchName"
 # Remove patch from remote machine
 ssh.exe -p $port "$user@$computer" "cd ~/Projects/Local/;rm -f $patchName"
...
# Remove patch from local machine
rm -f $patchName
